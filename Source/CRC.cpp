...
static const DWORD ExpectedSize = 0x0012FE00;
static const DWORD ExpectedCRC[] = {0xE5743F3E, 0xE1EAE253};
...
void CRC(const char* filepath) {
 ...
 if (h == INVALID_HANDLE_VALUE) Fail("Cannot open falloutw.exe for CRC check");
 ...
 if (!matchedCRC) {
  sprintf_s(buf, "You're trying to use sfall with an incompatible version of fallout\nWas expecting '" TARGETVERSION "'\n\nfalloutw.exe had an unexpected crc. Expected 0x%x but got 0x%x", ExpectedCRC[0], crc);
  Fail(buf);
 }
